...
{
	if(command[1] == NOTIFICATIONS)
    {
        if(command.size() == 2)
            Netflix :: get_instance() -> get_unread_notifications();
        else if(command[2] == READ && command.size() == 6)
        {
    ...
    else if(command[1] == MONEY && command.size()==2)
    {
...
void GET_Handler::show_all_movies(const vector<string> &command)
{
    string name;
    int min_year=0;
    int max_year=0;
